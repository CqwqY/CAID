...
### 优化（仅清凉版 caid-extension-lite）
- **选项页改版**：去掉常驻侧边栏（改为点击功能模块时从右侧滑出的抽屉）、去掉时间/搜索框区域。
- **监控改大卡片**：今日调用量、近 7 日调用量柱状图、最近任务由原窄侧卡改为顶部三栏横排大卡片，与下方功能模块统一卡片风格。
- **修复调用量统计不实时刷新**：`renderDashboard()` 此前仅在页面加载时跑一次，副驾在后台产生调用后数字不会更新。现新增 `chrome.storage.onChanged` 监听 `caidMemory`，数据变化即时重绘统计卡。
- **统计方案改为直接比对任务记录**：今日/近5日柱状图改由任务记录 `caidMemory.history` 的 `ts` 与今天日期比对得出（仅统计最近 5 天），不再依赖常驻埋点 `caidUsage`（该埋点在副驾不运行时无数据、易显示 0），并移除 background.js 中的 `bumpUsage` 埋点写入以消除无关存储开销。
