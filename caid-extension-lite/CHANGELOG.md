...
> ⚠️ 从 v0.3.19 起，本仓库**只维护纯净版 `caid-extension-lite`**，此文件即为唯一更新日志。完整版/旧版本日志保留在下方历史中，不再迭代。
...
### 重构（仅纯净版 caid-extension-lite）
- **选项页改版**：去掉常驻侧边栏、去掉时间/搜索框区域。
- **监控改大卡片**：今日调用量、近几日调用量柱状图、最近任务由原窄侧卡改为顶部三栏横排大卡片。
- **功能区块直接平铺在面板内**：代码片段 / 搜索历史 / 待办清单 / 副驾任务 / 设置 等不再需要点击小卡片弹出抽屉，而是作为卡片直接平铺展示在监控卡下方；功能内容一目了然、即开即用。
- **插件挂载也平铺为同级卡片**：插件（mount 视图）由原服务器监控下方的独立全宽区块，改为与功能区块同级的卡片网格，直接平铺在面板内，风格统一。
- **修复调用量统计不实时刷新**：`renderDashboard()` 此前仅在页面加载时跑一次，副驾在后台产生调用后数字不会更新。现新增 `chrome.storage.onChanged` 监听 `caidMemory`，数据变化即时重绘统计卡。
- **统计方案改为直接比对任务记录**：今日/近几日柱状图改由任务记录 `caidMemory.history` 的 `ts` 与今天日期比对得出（仅统计最近 5 天），不再依赖常驻埋点 `caidUsage`（该埋点在副驾不运行时无数据、易显示 0），并移除 background.js 中的 `bumpUsage` 埋点写入以消除无关存储开销。相关抽屉/模块网格逻辑（`renderModuleGrid` / `openSidebarComp` / `closeSidebarDrawer` / `DASH_MODULES`）已一并移除。

## [纯净版 v0.3.20] - 2026-08-22

### 重大重构（仅清凉版 caid-extension-lite）
- **工作台主区重排为仪表盘**：去掉原先的顶部快捷栏与搜索框（视觉隐藏，保留 DOM 以免破坏绑定）；新增左侧「今日调用量 + 近几日调用量柱状图（近 7 日）+ 最近任务」统计卡。
- **功能模块化平铺**：副驾任务 / 待办清单 / 代码片段 / 搜索历史 / 设置 / 错题本 / 检查更新 等以矩形卡片网格展示，点击直达对应侧边栏分区或设置弹窗页签。
- **调用量埋点**：background.js 新增 `caidUsage` 日统计（`{ 日期: { tasks, llm } }`，保留 90 天）。副驾任务启动（AGENT_ACTIVE）计 `tasks`，成功完成一次 LLM 请求（`CAID_LLM_FETCH`）计 `llm`；仪表盘据此渲染统计卡与柱状图。
- 新增模块渲染逻辑：`renderModuleGrid()` / `renderDashboard()` / `openSidebarComp()` / `openSettingsModal(tab)`；卡片点击「检查更新」会先打开设置弹窗再触发更新检查。
