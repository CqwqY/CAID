...
[OpenClaw](https://openclaw.ai/) 是优秀的开源系统级 AI 助手（本地运行、WhatsApp/Telegram 下指令、Shell/文件/Cron 全能），能力上限确实更高。CAID 走的是另一条路线，二者并非替代关系：

| 维度 | OpenClaw | CAID |
| ---- | -------- | ---- |
| 权限边界 | 整台电脑（文件读写、Shell、脚本执行） | 仅浏览器沙箱，碰不到文件系统与 Shell |
| 安装门槛 | Node ≥ 22 + 终端安装 + Gateway 配置，约 30 分钟 | 拖入 crx，约 30 秒 |
| 使用成本 | 必须自备 API Key，agent 重度调用持续烧钱 | 免费代理代持 Key，零配置零费用起步 |
| 交互范式 | 异步管家：发消息交任务，后台执行 | 同步副驾：新标签页即工作台，浏览时随时唤起 |
| 浏览器操作 | CDP 控制独立 Chromium（无登录态）；扩展中继需逐标签页手动连接 | MAIN world 原生注入，任意页面自动就位，支持跨页任务续传 |
| 出事代价 | 提示注入得手 = 整机沦陷（工信部已就配置风险发文提醒） | 爆炸半径锁定在单个浏览器会话 |

**简单说**：OpenClaw 适合愿意付费、懂配置、需要后台自动化（定时任务、文件整理、跨应用流程）的技术玩家；CAID 适合所有在浏览器里干活、想要零成本随用随走的用户。能力换安全、通用换专注，是刻意的设计取舍。
